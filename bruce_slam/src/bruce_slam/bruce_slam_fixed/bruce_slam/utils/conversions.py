...
# sensor_msgs.point_cloud2 is the ROS1 (rospy) location; ROS2 moved the
# same read_points/create_cloud* helpers into the standalone
# sensor_msgs_py package. The node/odometry/ping packages in this repo
# already run on rclpy (ROS2) talking to MAVROS, so we prefer that
# import and only fall back to the ROS1 path for environments still
# running this module against rospy (e.g. legacy bag post-processing).
try:
...
def r2g(ros_msg) -> gtsam.Pose3:
    ...
    # ros_msg._type (a string like "geometry_msgs/Pose") only exists on
    # ROS1 messages built via rospy - rclpy (ROS2) messages don't carry
    # it. isinstance() against the actual imported message classes
    # works identically under both, since the message classes/fields
    # themselves are unchanged between ROS1 and ROS2 for these types.
    if isinstance(ros_msg, PoseStamped):
        return r2g(ros_msg.pose)
    elif isinstance(ros_msg, Pose):
...
def r2n(ros_msg: Any) -> np.array:
    """Convert a ros message (OculusPing, Image, or PointCloud2) to a numpy array

    Args:
        ros_msg: the input sonar/image/cloud message
    ...
    # sonar_oculus.msg.OculusPing is a legacy Oculus-sonar message type
    # (this repo's live path is now the MAVROS/ROS2 pipeline driving a
    # Ping1D via PingManager, not an Oculus). It's kept here only for
    # replaying old Oculus bags, so it's checked defensively via
    # getattr rather than an isinstance/import that would otherwise
    # force every caller to have the sonar_oculus package installed.
    if getattr(ros_msg, "_type", None) == "sonar_oculus/OculusPing":
